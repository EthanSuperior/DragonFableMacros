...
# Okay now i want to add your pet dragon, probelem is the pet is another unit that exists, please allow users to be multiple Characters and add an index variable to determine whos turn it is 
# Now i want to add your pet dragon as a ally/pet this has cusomization as well, so please make the dragon from what it is trained in and what it's element is, its always a lvl 90 pet kid dragon: https://dragonfable-endgame.fandom.com/wiki/Dragon, all of the dragons effects basically actually target the player, not the dragon themselves.
