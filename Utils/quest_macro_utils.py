...
def masked_quest(classID, maskLocation, events=lambda: 1+1, loot=()):
    ...
    pX, pY = coords.pop(0)
    for x, y in coords: move_to((x, y), (classID, 'Quick'), events, loot)
...
def LootQuest():
    if m.ImageCheck('UI/ItemGet.png', (514, 126, 782, 184), 2):
        if m.ImageCheck(glob.glob('RejectedLoot/*.png'), (390, 241, 900, 296)):
            m_ui.ClickBtn('UI/RejectLoot.png', (592, 696, 714, 733), 'UI/Yes.png', (501, 443, 632, 494), False, 1)
...
def move_to(pos, moveSetID,events=lambda:1+1, loot=()):
    if type(pos) is str: pos = df_t.QuestLocations[pos]
    while True:
        m.Click(*pos)
        try:
            if not possibleBattle(moveSetID): break
            events()
...
def masked_from_folder(img_in, mask_folder):
    for file in glob.glob(f"./{mask_folder}/*.png"):
        with Image.open(file) as img_goal:
            ...
            if float(area / (diff_img.width * diff_img.height)) <= .2:
...
def largest_in_mask(diff_img, min = 0):
    ...
    if len(areas) == 0 or (m_area:=max(areas)) < min: return None
...
def map_it(x, y, rm_map, currHeading, wallDict):
    def str_map():
        ...
        for k, v in rm_map.items(): map_str[k[1] - ym_v][k[0] - xm_v] = v
        with open('../dungeonRoom.txt', 'wb') as file:
            file.write(('\n'.join([''.join(s) for s in map_str])).encode('UTF-8'))
    if 'North' in currHeading: y[0] -= 1
    if 'South' in currHeading: y[0] += 1
    if 'East' in currHeading: x[0] += 1
    if 'West' in currHeading: x[0] -= 1
...
def room_symbol(wallDict):
    ...
    return wall_ascii[(is_wall(img, 'North', wallDict), is_wall(img, 'East', wallDict),
                       is_wall(img, 'South', wallDict), is_wall(img, 'West', wallDict))]
